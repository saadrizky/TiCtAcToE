...
public class logic extends logicabstract {
	...
	public void instructGame() throws IOException {
		...
		System.out.println(
				"2. each location can only store 1 symbol, so make sure didn't choose the same box numerous time");
		System.out.println(
				"3. if you can make 3 input in the box SIDE BY SIDE or CROSS EACH OTHER, then you win, you can see the example below : ");
		System.out.println();
		System.out.println("------------------------------");
		System.out.println();

		char[][] papanwin1 = { { 'X', '|', 'X', '|', 'X' },
				{ '-', '+', '-', '+', '-' },
				{ ' ', '|', ' ', '|', ' ' },
				{ '-', '+', '-', '+', '-' },
				{ ' ', '|', ' ', '|', ' ' } };
	...
	public void winrate() throws IOException {
		...
		try {
			...
			while ((line = br.readLine()) != null) {// read setiap line
				String[] words = line.split(" ");// memisah kalimat berdasarkan "regex", karena "regex" yang kita input
													// adalah " "(spasi) maka setiap ada spasi, maka kalimat akan di
													// pisah (bisa baca di split)
				for (String word : words) {
					wordCounts.put(word, wordCounts.getOrDefault(word, 0) + 1);// memilih words yang udah di split
																				// menjadi "key" dan value nya menjadi
																				// "jumlah karakter"
				} // menaruh di words setiap kata yg di ambil lalu di store di hashmap dengan
					// format, 1 kata dan jumlah karakterperkata, untuk value waktu di anggap 1
					// sekaligus
			}

			// setiap line yang dipisah menjadi regex masing2 akan di read dan ditulis satu
			// persatu, lalu kalimat tersebut akan di set sebagai key, dan value nya adalah
			// jumlah satupersatu kalimat key dibaca, yg mana valuenya pun akan bertambah +1
			// setiap kali key tersebut muncul/terdeteksi dalam Recordwin.txt setiap
			// barisnya
